...
The precompiled server binary was built specifically for:
- **Architecture:** `aarch64` (ARM 64-bit)
- **Operating System:** Linux (Debian-based)
- **Kernel:** 6.12.62+ or compatible

This binary was compiled using **Nuitka** and **PyInstaller** on a Raspberry Pi system and will **only function on compatible ARM64 Linux systems**.

**This binary is not available for other architectures** (x86_64, armv7l, etc.).
...
```
BlackBerryC2 v1.7 - Herramienta de administración remota encriptada

Comandos del Servidor:
  list                   -> Lista conexiones con estadísticas y hostname.
  select <ID>            -> Interactúa con una sesión.
  all <comando>          -> Envía comando a todos los clientes.
  ...
  fingerprint            -> Muestra fingerprint RSA del servidor.
  proxy-help             -> Ayuda del proxy TLS.
  log                    -> Imprime el log del servidor.
  block <IP>             -> Bloquea IP permanentemente.
  unblock <IP>           -> Desbloquea IP (permanente y temporal).
  blocklist              -> Muestra IPs bloqueadas.
  clean                  -> Limpia archivos de log.

  E <comando>            -> Ejecuta comando con os.system.
  <cualquier comando>    -> Se ejecuta localmente en el servidor.
